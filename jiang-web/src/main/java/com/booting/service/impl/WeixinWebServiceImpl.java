...
public class WeixinWebServiceImpl {
	...
	private final String userInfoUrl = "https://api.weixin.qq.com/sns/userinfo?access_token=%s&openid=%s";
	private static Map<String, AccessToken> accessTokens = new HashMap<>();
	private static Map<String, AccessToken> accessTokensByOpenId = new HashMap<>();
...
//		}
	...
	public AccessToken getAccessToken(String code) throws IOException {
		...
		if (null == accessToken) {
			...
			accessTokensByOpenId.put(accessToken.getOpenid(), accessToken);
...
//		accessToken.setSubscribe(WeiXinUtil.isSubscribe(accessToken.getOpenid()));
//		System.out.println(accessToken);
	...
	public WxUserInfo getUserInfo(String openId) throws IOException {
		...
		AccessToken accessToken = accessTokensByOpenId.get(openId);
		if (null == accessToken) {
			String temp = requestGet(String.format(userInfoUrl, accessToken, openId));
			userInfo = mapper.readValue(temp, WxUserInfo.class);
		}
		return userInfo;
	}
}