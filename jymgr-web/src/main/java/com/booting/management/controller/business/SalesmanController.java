...
import com.booting.service.impl.SalesmanWebService;
...
public class SalesmanController {
  ...
  @Autowired
  private SalesmanWebService salesmanWebService;

  @ResponseBody
  @RequestMapping(value = "/list", method = RequestMethod.POST)
  public Map<String, Object> list(SalesmanDTO salesmanDTO, Integer page, Integer rows, String sord, String sidx) {
    ...
    PageList<SalesmanDTO> pageList = salesmanWebService.getListForPageSalesman(queryParam, SalesmanDTO.class);
  ...
  public String editBefore(@PathVariable Long memberId, Model model) {
    SalesmanDTO salesmanDTO = this.salesmanWebService.getSalesman(memberId);
    model.addAttribute("salesmanDTO", salesmanDTO);
    PictureChildType[] types = BusinessConst.createTeachingCerts();
    model.addAttribute("types", types);
    return "management/salesman/editSalesman";
  }
  ...
  public String viewBefore(@PathVariable Long memberId, Model model) {
    ...
    PictureChildType[] types = BusinessConst.createTeachingCerts();
    model.addAttribute("types", types);
    return "management/salesman/viewSalesman";
  }

  @ResponseBody
  @RequestMapping(value = "/edit", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
  public ResultMessage edit(@RequestBody SalesmanDTO salesmanDTO) {
    ResultMessage resultMessage = null;
    try {
      this.salesmanWebService.updateSalesman(salesmanDTO);
  ...
  public ResultMessage add(@RequestBody SalesmanDTO salesmanDTO) {
    ResultMessage resultMessage = null;
    try {
      this.salesmanWebService.saveSalesman(salesmanDTO);
  ...
  public ResultMessage enabled(@RequestParam String ids) {
    ResultMessage resultMessage = null;
    try {
      this.salesmanWebService.enabledSalesman(ids);
  ...
  public ResultMessage disabled(@RequestParam String ids) {
    ResultMessage resultMessage = null;
    try {
      this.salesmanWebService.disabledSalesman(ids);
  ...
  public ResultMessage delete(@RequestParam String ids) {
    ResultMessage resultMessage = null;
    try {
      this.salesmanWebService.deleteSalesman(ids);
