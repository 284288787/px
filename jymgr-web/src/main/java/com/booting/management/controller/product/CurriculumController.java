...
import com.booting.service.impl.CurriculumWebService;
...
public class CurriculumController{
	...
	@Autowired
	private CurriculumWebService curriculumWebService;

	@ResponseBody
	@RequestMapping(value = "/list", method = RequestMethod.POST)
	public Map<String, Object> list(CurriculumDTO curriculumDTO, Integer page, Integer rows, String sord, String sidx){
		...
		PageList<CurriculumDTO> pageList = curriculumWebService.getListForPageCurriculum(queryParam, CurriculumDTO.class);
	...
	@RequestMapping(value = "/editBefore/{curriculumId}", method = RequestMethod.GET, produces = MediaType.TEXT_HTML_VALUE)
	public String editBefore(@PathVariable Long curriculumId, Model model){
	  CurriculumDTO curriculumDTO = this.curriculumWebService.getCurriculum(curriculumId);
	  model.addAttribute("curriculumDTO", curriculumDTO);
	  return "management/curriculum/editCurriculum";
	}

	@RequestMapping(value = "/viewBefore/{curriculumId}", method = RequestMethod.GET, produces = MediaType.TEXT_HTML_VALUE)
	public String viewBefore(@PathVariable Long curriculumId, Model model){
		CurriculumDTO curriculumDTO = this.curriculumWebService.getCurriculum(curriculumId);
		model.addAttribute("curriculumDTO", curriculumDTO);
		return "management/curriculum/viewCurriculum";
	}

	@ResponseBody
	@RequestMapping(value = "/edit", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	public ResultMessage edit(@RequestBody CurriculumDTO curriculumDTO){
		ResultMessage resultMessage = null;
		try {
			this.curriculumWebService.updateCurriculum(curriculumDTO);
	...
	public ResultMessage add(@RequestBody CurriculumDTO curriculumDTO){
		ResultMessage resultMessage = null;
		try {
			this.curriculumWebService.saveCurriculum(curriculumDTO);
	...
	public ResultMessage enabled(@RequestParam String ids){
		ResultMessage resultMessage = null;
		try {
			this.curriculumWebService.enabledCurriculum(ids);
	...
	public ResultMessage disabled(@RequestParam String ids){
		ResultMessage resultMessage = null;
		try {
			this.curriculumWebService.disabledCurriculum(ids);
	...
	public ResultMessage delete(@RequestParam String ids){
		ResultMessage resultMessage = null;
		try {
			this.curriculumWebService.deleteCurriculum(ids);
