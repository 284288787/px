...
<body>
	<div class="edit-container"> 
		<table width="80%" border="0" cellspacing="0" cellpadding="0" class="gy_table">
			<tr>
				<td width="160px"><strong>活动类型：</strong></td><td>${activityDTO.typeName}</td>
			</tr>
			<tr>
				<td><strong>活动名称：</strong></td><td>${activityDTO.name}</td>
			</tr>
			<tr>
				<td><strong>活动价格：</strong></td><td><fmt:formatNumber value="${activityDTO.price/100}" pattern="#0.00"></fmt:formatNumber></td>
			</tr>
			<tr>
				<td><strong>有效性：</strong></td><td>
				<c:if test="${activityDTO.state == 1}">永久有效</c:if>
				<c:if test="${activityDTO.state == 2}">限时有效</c:if>
				</td>
			</tr>
			<c:if test="${activityDTO.state == 2}">
			<tr>
				<td><strong>生效时间：</strong></td><td><fmt:formatDate value="${activityDTO.effectiveTime}" pattern="yyyy-MM-dd"/></td>
			</tr>
			<tr>
				<td><strong>失效时间：</strong></td><td><fmt:formatDate value="${activityDTO.failureTime}" pattern="yyyy-MM-dd"/></td>
			</tr>
			</c:if>
			<tr>
			...
			<tr>
				<td><strong>活动简介：</strong></td><td height="120px">${activityDTO.intro}</td>
			</tr>
			<tr>
				<td><strong>活动详介：</strong></td><td height="300px">${activityDTO.content}</td>
			</tr>
			<tr>
				<td colspan="2">
					<div class="btnGroup2"> 
