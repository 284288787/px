...
public class MobileApiAspect {
	...
	public Object around(ProceedingJoinPoint joinPoint) {
		...
		if (null != args && args.length > 0 && DesUtil.encrypt_args) {
			...
			ParamHandler.strToObj(temp, ParamHandler.objectMapper.getTypeFactory().constructType(type));
			args[0] = temp;
		}
		...
		try {
			...
			if (null != object && StringUtils.isNotBlank(object.toString())) {
				ApiResult apiResult = ParamHandler.strToObj(object.toString(), ApiResult.class);
				if (null != apiResult) {
...
//			ex.printStackTrace();
		...
		String result = ParamHandler.objToString(resultMessage);
