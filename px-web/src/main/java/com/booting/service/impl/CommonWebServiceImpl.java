...
public class CommonWebServiceImpl implements CommonWebService {
	...
	public String createAreaJson() {
		...
		String areas = ParamHandler.objToString(provinces);
		return areas;
	}
	...
	public String createBusinessJson() {
		...
		return ParamHandler.objToString(businesses);
	}
	...
	public MemberDTO getMemberInfo(MemberDTO memberDTO) throws ArgsException {
		...
		if (null != member) {
			...
			TeacherDTO teacher = getTeacher(member.getMobile());
			if (null != teacher && null != teacher.getTeacherId()) {
				member.setTeacher(true);
			}
			KindergartenDTO school = getSchool(member.getMobile());
			if (null != school && null != school.getSchoolId()) {
				member.setSchoolMgr(true);
			}
			CoachDTO coach = getCoachByMobile(member.getMobile());
			if (null != coach && null != coach.getCoachId()) {
				member.setCoach(true);
			}
			List<StudentDTO> students = getStudents(member.getMobile());
			if (null != students && ! students.isEmpty()) {
				member.setParent(true);
				member.setChildren(true);
			}else{
				...
				if (null != parent && null != parent.getParentId()) {
					...
					List<StudentParentRelationDTO> relations = this.kindergartenFacade.getStudentParentRelationList(param);
	...
	public TeacherDTO getTeacher(String mobile){
		TeacherDTO param = new TeacherDTO();
		param.setTeacherMobile(mobile);
		TeacherDTO teacher = kindergartenFacade.getTeacher(param);
		return teacher;
	}
	
	public KindergartenDTO getSchool(String mobile){
		KindergartenDTO param = new KindergartenDTO();
		param.setMobile(mobile);
		KindergartenDTO kindergarten = kindergartenFacade.getKindergarten(param);
		return kindergarten;
	}
	
	public List<StudentDTO> getStudents(String mobile){
		StudentDTO param = new StudentDTO();
		param.setGuardianMobile(mobile);
		List<StudentDTO> students = kindergartenFacade.getStudentList(param);
		return students;
	}
	...
	public ParentDTO getParent(String mobile){
		...
		param.setMobile(mobile);
		ParentDTO parent = kindergartenFacade.getParent(param);
		return parent;
	}
