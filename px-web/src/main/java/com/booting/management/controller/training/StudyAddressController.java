/** create by auto at 2019-02-28 10:45:08**/
...
@Controller
@RequestMapping("/studyAddress")
public class StudyAddressController{
	...
	public Map<String, Object> list(StudyAddressDTO studyAddressDTO, Integer page, Integer rows, String sord, String sidx){
		...
		PageList<StudyAddressDTO> pageList = studyAddressWebService.getListForPageStudyAddress(queryParam, StudyAddressDTO.class);
	...
	@RequestMapping(value = "/editBefore/{addrId}", method = RequestMethod.GET, produces = MediaType.TEXT_HTML_VALUE)
	public String editBefore(@PathVariable Long addrId, Model model){
		StudyAddressDTO studyAddressDTO = this.studyAddressWebService.getStudyAddress(addrId);
		model.addAttribute("studyAddressDTO", studyAddressDTO);
		return "management/studyAddress/editStudyAddress";
	}

	@ResponseBody
	@RequestMapping(value = "/edit", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	public ResultMessage edit(@RequestBody StudyAddressDTO studyAddressDTO){
		ResultMessage resultMessage = null;
		try {
			this.studyAddressWebService.updateStudyAddress(studyAddressDTO);
	...
	public ResultMessage add(@RequestBody StudyAddressDTO studyAddressDTO){
		ResultMessage resultMessage = null;
		try {
			this.studyAddressWebService.saveStudyAddress(studyAddressDTO);
	...
	public ResultMessage enabled(@RequestParam String ids){
		ResultMessage resultMessage = null;
		try {
			this.studyAddressWebService.enabledStudyAddress(ids);
	...
	public ResultMessage disabled(@RequestParam String ids){
		ResultMessage resultMessage = null;
		try {
			this.studyAddressWebService.disabledStudyAddress(ids);
	...
	public ResultMessage delete(@RequestParam String ids){
		ResultMessage resultMessage = null;
		try {
			this.studyAddressWebService.deleteStudyAddress(ids);
