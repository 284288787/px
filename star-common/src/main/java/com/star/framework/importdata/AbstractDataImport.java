...
public abstract class AbstractDataImport<T> {
	...
	public AbstractDataImport(byte[] bs, String filename) {
		super();
		workbook = DataExportUtil.getWorkbook(bs, filename);
	}
	...
	public ImportResult importData(Sheet sheet){
		...
		ImportResult importResult = new ImportResult(empty, heads, errorRecords, successRecords);
		return importResult;
	}
	...
	public void save(ImportResult importResult) throws ArgsException {
		if (null == importResult) {
			throw new ArgsException(FailureCode.ERR_000);
		}
		...
		for (List<FieldValue> record : successList) {
			...
			String json = ParamHandler.objToString(line);
			data.add(ParamHandler.strToObj(json, clzz));
		}
