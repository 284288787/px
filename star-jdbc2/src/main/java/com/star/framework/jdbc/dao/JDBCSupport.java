...
public class JDBCSupport<E, D> implements JDBCBaseDAO<E, D> {
	...
	public D getById(Serializable id) {
		...
		D entity = null;
		try {
			entity = jdbcTemplate2.queryForObject(selectSql, paramMap, new RowMapperFactory<D>(getDTOClass()).getRowMapper());
		} catch (Exception e) {
		}
...
//		System.out.println("select Object: " + sql + "\n" + paramMap);
		long beginDate = System.currentTimeMillis();
		NamedParameterJdbcTemplate jdbcTemplate2 = new NamedParameterJdbcTemplate(jdbcTemplate);
		T object = null;
		try {
			object = jdbcTemplate2.queryForObject(sql, paramMap, new RowMapperFactory<T>(clazz).getRowMapper());
	...
	public PageList<D> queryForPage(String countSqlId, String listSqlId, QueryParam queryParam){
		...
		if (pageSize == -1) {
          ...
          pageList = new PageList<>(dataList);
        }else{
	...
	public <T> PageList<T> queryForPage(String countSqlId, String listSqlId, QueryParam queryParam, Class<T> clazz){
		...
		if (totalRecord > 0) {
			...
			paramMap.put("orderBy", queryParam.getOrderBy());
			paramMap.put("orderType", queryParam.getOrderType());
			List<T> dataList = queryForList(listSqlId, paramMap, clazz);
			pageList = new PageList<>(pageNo, pageSize, totalRecord, totalPage, dataList);
		}
		return pageList;
	}
	
	public <B> List<B> queryForList(String sqlId, Map<String, Object> paramMap, RowMapper<B> rowMapper) {
		SqlBean sqlBean = this.xmlResource.getSQL(sqlId);
		String sql = FreeMakerParser.process(paramMap, sqlBean.getContent(), sqlId);
...
//		System.out.println("queryForList: " + sql + "\n" + paramMap);
		long beginDate = System.currentTimeMillis();
		NamedParameterJdbcTemplate jdbcTemplate2 = new NamedParameterJdbcTemplate(jdbcTemplate);
		List<B> resultList = jdbcTemplate2.query(sql, paramMap, rowMapper);
	...
	public <T> Map<String, Object>[] toMap(List<T> objects){
		if (null == objects) {
			return null;
		}
		@SuppressWarnings("unchecked")
		Map<String, Object>[] params = new HashMap[objects.size()];
		for (int i = 0; i < objects.size(); i++) {
			T t = objects.get(i);
			if (null != t) {
				Map<String, Object> map = new HashMap<>();
				CglibBeanUtils.addToMap(t, map);
				params[i] = map;
			}
		}
		return params;
